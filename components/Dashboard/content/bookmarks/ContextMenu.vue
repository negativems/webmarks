<template>
  <div class="context-menu absolute z-10 h-0 w-[150px] rounded-lg bg-accent-light delay-200 duration-100 dark:bg-accent-dark">
    <div class="flex flex-col gap-2">
      <button
        class="py-2 rounded-t-lg btn btn-primary hover:bg-accent"
        @click="handleFavouriteClick()"
      >
        Favourite
      </button>
      <button
        class="py-2 btn btn-primary hover:bg-accent"
...
const bookmark = computed(
	() => useBookmark().find(props.bookmarkId) as Bookmark,
);
...
onMounted(() => {
	...
	const contextMenuButtons = $$("button", $contextMenu) as HTMLButtonElement[];
	for (const button of contextMenuButtons) {
	...
	document?.addEventListener("contextmenu", (e: any) => {
		const rect = $("div.bookmarks")?.getBoundingClientRect();
