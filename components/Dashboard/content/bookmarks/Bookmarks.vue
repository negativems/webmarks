<template>
  ...
  />
  <div
    class="relative grid gap-5 bookmarks"
    :class="{
  ...
  >
    <div
      ...
      class="absolute h-auto text-center left-1/2 top-1/2"
    >
      Loading...
    </div>
    ...
    >
      <StarIcon
        ...
        color="#fcec03"
        fill="#fcec03"
        class="absolute z-10 scale-75 star-icon left-1 top-1"
      />
...
import type { Bookmark, BookmarkDisplayFormat } from "@/types/types";
...
const prop = defineProps(["search", "sort"]);
...
const isRow = computed(
	() =>
		bookmarkStore.bookmarkDisplayFormat === ("rows" as BookmarkDisplayFormat),
);
...
const hoverBookmarkId = ref(undefined as number | undefined);
...
const bookmarks = computed(() => {
	...
	const selectedTags = bookmarkStore.selectedTags;
		...
		if (useBookmark().bookmarkFilterTagType === "restrictive") {
			...
			result = result.filter((bookmark) =>
				selectedTags.every((tag) =>
					bookmark.tags?.some((bookmarkTag) => bookmarkTag.name === tag),
				),
			);
		} else {
			// At least one of the selected tags must be in the bookmark
			result = result.filter((bookmark) =>
				bookmark.tags?.some((tag) => selectedTags.includes(tag.name)),
			);
		}
	}
	...
	// Filter by search
	if (search.value) {
		const titleIncludes = (bookmark: Bookmark) =>
			bookmark.title.toLowerCase().includes(search.value.toLowerCase());
		const urlIncludes = (bookmark: Bookmark) =>
			bookmark.url.toLowerCase().includes(search.value.toLowerCase());
		result = result.filter(
			(bookmark: any) => titleIncludes(bookmark) || urlIncludes(bookmark),
		);
	}
