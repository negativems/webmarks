<template>
  <div class="flex" :class="{
    ...
    <DashboardContentBookmarksHeader class="header flex gap-5" v-if="hasBookmarks" v-model:search="search" :hasBookmarks="hasBookmarks" :format="displayFormat" :filterTagsType="filterTagsType" />

    <DashboardContentBookmarks :search="search" />

    <div class="no-bookmarks inline-flex flex-col items-center gap-5 rounded-2xl border-8 border-dashed p-10 text-center backdrop-blur-3xl" v-if="useBookmark().bookmarksLoaded && !hasBookmarks">
