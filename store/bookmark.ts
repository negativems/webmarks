...
import type {
  Bookmark,
  BookmarkDisplayFormat,
  BookmarkDraft,
  BookmarkFilterTagType,
  Tag,
} from "~/types/types";
import { defineStore } from "pinia";
...
export const useBookmark = defineStore("bookmark", {
  ...
  getters: {
    ...
    getTags: (state) => (bookmarkId?: number): Tag[] => {
      ...
      const filteredTags = state.bookmarks
        // Filter bookmarks by id
        .filter((bookmark) => bookmarkId === undefined || bookmark.id === bookmarkId)
        // Get all tags from all bookmarks
...
>>>>>>> Stashed changes
    ...
    exists:
      (state) =>
        ({ title = "", url = "" }: { title?: string; url?: string }): boolean => {
          // Check that there is a title or url
          if (!title && !url) {
            console.error(
              "There was an error checking if a bookmark exists: no title or url was provided",
            );
            return false;
          }
  ...
  actions: {
    ...
    existsTag(name: string, bookmark?: Bookmark): boolean {
      ...
      return this.bookmarks.some((bookmark) =>
        bookmark.tags?.some((tag) => tag.name === name),
      );
    },
    ...
    load({
      mostUsed = false,
      favourite = false,
    }: { mostUsed?: boolean; favourite?: boolean } = {}) {
      this.bookmarksLoaded = false;
      if (process.client) {
        const user = useSupabaseUser().value;
        const supabase = useSupabaseClient();
      ...
      if (process.client) {
        ...
        if (user === null) {
          ...
          localStorage.setItem(
            "bookmarks",
            JSON.stringify([...this.bookmarks, bookmark]),
          );
          this.bookmarks.push(bookmark as Bookmark);
          return;
        }
      ...
      if (process.client) {
        ...
        if (user === null) {
          for (let i = 0; i < bookmarks.length; i++) {
            bookmarks[i].id = this.localBookmarksIndex++;
            bookmarks[i].redirects = 0;
          }
        ...
        for (let i = 0; i < chunks.length; i++) {
          supabase
            .from("bookmarks")
            .insert(chunks[i] as any)
            .then(() => {
              this.bookmarks.push(...chunks[i]);
            });

          this.insertingBookmarks += chunks[i].length;
        }
    ...
    loadBookmarkDisplayFormat() {
      if (
        process.client &&
        localStorage.getItem("bookmarkDisplayFormat") !== null
      ) {
        this.bookmarkDisplayFormat = localStorage.getItem(
          "bookmarkDisplayFormat",
        ) as BookmarkDisplayFormat;
      }
    },
     ...
     */
    toggleBookmarkDisplayFormat() {
      this.bookmarkDisplayFormat =
        this.bookmarkDisplayFormat === "rows" ? "cards" : "rows";
    ...
    loadBookmarkFilterTagType() {
      if (
        process.client &&
        localStorage.getItem("bookmarkFilterTagType") !== null
      ) {
        this.bookmarkFilterTagType = localStorage.getItem(
          "bookmarkFilterTagType",
        ) as BookmarkFilterTagType;
      }
    },

    toggleFilterTagType() {
      this.bookmarkFilterTagType =
        this.bookmarkFilterTagType === "restrictive"
          ? "individual"
          : "restrictive";
    ...
    addClickTo(bookmarkId: number) {
      ...
      if (process.client) {
        ...
        const redirects = (this.bookmarks.find((bookmark) => bookmark.id === bookmarkId)?.redirects || 0) + 1 || 1;
        const result = { id: bookmarkId, redirects } as never;
        supabase
          .from("bookmarks")
          .update(result)
    ...
    addTag(
      ...
      if (this.getTags(bookmarkId).some((tag) => tag.name === tagName)) {
        console.error(
          "Error adding tag to bookmark: tag already exists",
          bookmarkId,
          tagName,
          color,
        );
    ...
    deleteTag(tagName: string, bookmarkId?: number) {
      ...
      if (process.client) {
        ...
        if (user === null) {
          let bookmarks = this.bookmarks.filter((bookmark) =>
            bookmark.tags.some((tag) => tag.name === tagName),
          );
          if (bookmarkId) {
          ...
          for (let i = 0; i < bookmarks.length; i++) {
            bookmarks[i].tags = bookmarks[i].tags.filter((tag) => tag.name !== tagName);
          }
